...
export class BuyForm extends React.PureComponent<OrderFormProps, OrderFormState> {
    ...
    return (
      <form onSubmit={()=> this.handleEnterPress}> 
        ...
        <S.ContentAction>
          ...
          />
          <div>
          {userLoggedIn && `I will receive: ${orderType === 'Market' ? '~;' : ''} ${Decimal.format(total, currentMarketAskPrecision + currentMarketBidPrecision, ',')} ${to.toUpperCase()}`}
          </div>
        </S.ContentAction>
      </form>
    )
