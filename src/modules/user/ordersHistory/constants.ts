export const ORDERS_HISTORY_FETCH = "ordersHistory/FETCH";
export const ORDERS_HISTORY_DATA = "ordersHistory/DATA";
export const ORDERS_HISTORY_ERROR = "ordersHistory/ERROR";
export const ORDERS_CHANNEL_FETCH = "ordersHistory/CHANNEL_FETCH";
export const ORDER_CHANNEL_UPDATE_DATA = "ordersHistory/CHANNEL_UPDATE_DATA";
export const ORDER_UPDATE_ACCEPTED = "ordersHistory/CHANNEL_UPDATE_ACCEPTED";
export const ORDER_UPDATE_FILLED = "ordersHistory/CHANNEL_UPDATE_FILLED";
export const ORDER_UPDATE_PARTIALLYFILLED = "ordersHistory/CHANNEL_UPDATE_PARTIALLYFILLED";
