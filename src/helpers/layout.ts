...
export const getStaticHeight = () => {
  const header = process.browser && document.getElementsByTagName("header")[0];
  const headerHeight = header ? header.clientHeight : 0;
  const headerContainer =
    process.browser &&
    document.getElementsByClassName("pg-trading-header-container")[0];
  const headerContainerHeight = headerContainer
...
export const gridUnitsToPixels = (
  gridUnit: number,
  rowHeight: number,
  margin: number
) => {
...
export const pixelsToGridUnits = (
  pixels: number,
  rowHeight: number,
  margin: number
) => {
...
const getLayouts = () => {
  ...
  const gridHeight = Math.max(
    minGridHeight,
    process.browser && window.innerHeight - staticHeight - margin * 3
  );
  ...
  const marketsHeight =
    gridHeight >= minGridHeight
      ? pixelsToGridUnits(gridHeight - margin, rowHeight, margin) -
        orderHeight +
        margin -
        3.3
      : minMarketsHeight;
