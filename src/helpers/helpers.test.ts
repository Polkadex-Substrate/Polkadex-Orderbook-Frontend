...
describe("Helpers", () => {
  ...
  it("Should right format BCH address", () => {
    ...
    expect(
      helpers.formatCCYAddress("BTC", "2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2cL")
    ).toEqual("2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2cL");
    expect(helpers.formatCCYAddress("BCH", "")).toEqual("");
    expect(
      helpers.formatCCYAddress("BCH", "2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2c0")
    ).toEqual("");
    expect(
      helpers.formatCCYAddress("BCH", "2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2c")
    ).toEqual("");
    expect(
      helpers.formatCCYAddress("BCH", "2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2cLa")
    ).toEqual("");
    expect(
      helpers.formatCCYAddress("BCH", "2NDJKBjTcoPRXTnkAbcyeqLwXQfdoeug2cL")
    ).toEqual("bchtest:prdlsx39pn5v6yxmcgaqgeg6zd48snqaay69chcgwx");
  });
  ...
  it("Should return correct deposit status color", () => {
    expect(helpers.setDepositStatusColor("accepted")).toEqual(
      "var(--system-green)"
    );
    expect(helpers.setDepositStatusColor("collected")).toEqual(
      "var(--system-green)"
    );
    expect(helpers.setDepositStatusColor("submitted")).toEqual("");
    expect(helpers.setDepositStatusColor("canceled")).toEqual(
      "var(--system-red)"
    );
    expect(helpers.setDepositStatusColor("rejected")).toEqual(
      "var(--system-red)"
    );
  });
  ...
  it("Should return correct withdraw status color", () => {
    ...
    expect(helpers.setWithdrawStatusColor("canceled")).toEqual(
      "var(--system-red)"
    );
    expect(helpers.setWithdrawStatusColor("accepted")).toEqual(
      "var(--system-green)"
    );
    expect(helpers.setWithdrawStatusColor("suspected")).toEqual("");
    expect(helpers.setWithdrawStatusColor("rejected")).toEqual(
      "var(--system-red)"
    );
    expect(helpers.setWithdrawStatusColor("processing")).toEqual("");
    expect(helpers.setWithdrawStatusColor("succeed")).toEqual(
      "var(--system-green)"
    );
    expect(helpers.setWithdrawStatusColor("failed")).toEqual(
      "var(--system-red)"
    );
  ...
  it("Should return correct locale date", () => {
    expect(
      helpers.localeDate("2018-11-29T16:54:46+01:00", "fullDate", "Europe/Kiev")
    ).toBe("29-11-2018 17:54:46");
    expect(
      helpers.localeDate("2018-11-12T16:55:12-01:00", "fullDate", "Europe/Kiev")
    ).toBe("12-11-2018 19:55:12");
    expect(
      helpers.localeDate(
        "2018-11-29T16:54:46+01:00",
        "shortDate",
        "Europe/Kiev"
      )
    ).toBe("29-11-2018 17:54");
    expect(
      helpers.localeDate(
        "2018-11-12T16:55:12-01:00",
        "shortDate",
        "Europe/Kiev"
      )
    ).toBe("12-11-2018 19:55");
    expect(
      helpers.localeDate("2018-11-29T16:54:46+01:00", "time", "Europe/Kiev")
    ).toBe("17:54:46");
    expect(
      helpers.localeDate("2018-11-12T16:55:12-01:00", "time", "Europe/Kiev")
    ).toBe("19:55:12");
  });
  ...
  it("should extract a segment part of a url", () => {
    ...
    expect(helpers.getUrlPart(2, "/trading/BTCUSD")).toEqual("BTCUSD");
    expect(helpers.getUrlPart(2, "/trading/btcusd/something")).toEqual(
      "btcusd"
    );
    expect(helpers.getUrlPart(2, "/trading/btcusd#something")).toEqual(
      "btcusd"
  ...
  it("Should correctly convert date", () => {
    expect(helpers.dateTo12HFormat("2019-01-22T15:18:33.000Z")).toBe(
      "Jan 22, 2019 3:18 PM"
    );
    expect(helpers.dateTo12HFormat("2019-01-22T08:08:36.000Z")).toBe(
      "Jan 22, 2019 8:08 AM"
    );
  });
  ...
  it("Should return correct params", () => {
    expect(
      helpers.getHistorySagaParam({ page: 0, limit: 25, type: "deposits" })
    ).toBe("page=1&limit=25");
    expect(
      helpers.getHistorySagaParam({ page: 0, limit: 10, type: "withdraws" })
    ).toBe("page=1&limit=10");
    expect(
      helpers.getHistorySagaParam({ page: 0, limit: 5, type: "trades" })
    ).toBe("page=1&limit=5");
    expect(
    ...
    expect(
      ...
      "page=1&limit=25&market=btcusd&time_from=1557878400&time_to=1557964799"
    );
    expect(
      helpers.getHistorySagaParam({
        page: 0,
  ...
  it("Should return correct params", () => {
    ...
    expect(helpers.getUserAgent("PostmanRuntime/7.6.0")).toBe(
  ...
  describe("truncateMiddle", () => {
    it("return correct value", () => {
      ...
      expect(helpers.truncateMiddle("return correct value", 10)).toBe(
        "retu...lue"
      );
      expect(helpers.truncateMiddle("return correct value", 30)).toBe(
  ...
  describe("Helpers", () => {
    it("getCountdownDate", () => {
      expect(helpers.getCountdownDate("2021-04-01T15:55:00.000Z")).toBe(
        "00:00:00"
      );
      expect(helpers.getCountdownDate("2021-04-01T15:55:00.000Z", "5m")).toBe(
        "00:00:00"
      );
    });
  });
