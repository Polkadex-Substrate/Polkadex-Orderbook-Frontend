...
describe("Describe validatePriceStep filter", () => {
  it("should validate significant_digits filter", () => {
    ...
    expect(
      validatePriceStep(
        100,
        [{ type: "significant_digits", digits: 5 }].map(buildFilterPrice)
      )
  ...
  it("should validate custom_price_steps filter", () => {
    ...
    expect(validatePriceStep(100.001, filters)).toEqual({
      valid: true,
      priceStep: 0,
    });
    ...
    expect(validatePriceStep(99.99, filters)).toEqual({
      valid: false,
      priceStep: 1,
    });
    ...
    expect(validatePriceStep(110, filters)).toEqual({
      valid: true,
      priceStep: 0,
    });
    expect(validatePriceStep(111, filters)).toEqual({
      valid: false,
      priceStep: 10,
    });
  });
  ...
  it("should validate multiple filters", () => {
    ...
    expect(validatePriceStep(1, filters)).toEqual({
      valid: true,
      priceStep: 0,
    });
    expect(validatePriceStep(5.6, filters)).toEqual({
      valid: false,
      priceStep: 1,
    });
    expect(validatePriceStep(110, filters)).toEqual({
    ...
    expect(validatePriceStep(1110, filters)).toEqual({
      valid: false,
      priceStep: 100,
    });
    expect(validatePriceStep(111000, filters)).toEqual({
      valid: false,
      priceStep: 10000,
    });
  });
