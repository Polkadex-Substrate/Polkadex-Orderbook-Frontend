...
export class FilterPriceCustomStep implements FilterPrice {
  ...
  public validatePriceStep(price: number): PriceValidation {
    ...
    const nextIndex = this.filter.rules.findIndex(
      (step) => step && +step.limit > price
    );
    const zeroIndex = this.filter.rules.findIndex(
      (step) => step && +step.limit === 0
    );
    ...
    if (currentLimit) {
      ...
      const targetCoefficient =
        priceDecimals > stepDecimals ? priceDecimals : stepDecimals;
      const validStepCoefficient = targetCoefficient
        ? 10 ** targetCoefficient
        : 1;
      ...
      currentValidation.priceStep = !currentValidation.valid
        ? +currentLimit.step
        : 0;
    }
