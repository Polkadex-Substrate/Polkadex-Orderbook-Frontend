import {
  FilterPriceCustomStep,
  MarketFilterCustomStep,
} from "./filterPriceCustomStep";
import {
...
export const buildFilterPrice = (
  filter: MarketFilterSignificantDigit,
): FilterPrice => {
  switch (filter.type) {
    case "significant_digits":
      return new FilterPriceSignificantDigit(
        filter as MarketFilterSignificantDigit,
      );
    case "custom_price_steps":
      return new FilterPriceCustomStep(
        filter as unknown as MarketFilterCustomStep,
      );
    default:
