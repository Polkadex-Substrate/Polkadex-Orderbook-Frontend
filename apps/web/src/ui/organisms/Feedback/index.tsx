...
const Feedback = () => {
  ...
  const answerHeight =
    formik.values.answer === false ? answerRef.current?.scrollHeight : 0;
  const userFriendlyHeight =
    formik.values.userFriendly === false
          ...
          <S.QuestionWrapper>
            ...
            <S.SwitchWrapper>
              <S.Switch onClick={() => handleAnswer(false)}>
                <S.SwitchHandle
                  active={formik.values.answer === false}
                ></S.SwitchHandle>
            ...
            <S.Comment ref={answerRef} maxHeight={answerHeight}>
              <S.Question>
                Please state your query below and our Support Executive will get
                back to you
              </S.Question>
              <S.Input
            ...
            <S.SwitchWrapper>
              <S.Switch onClick={() => handleUserFriendly(false)}>
                <S.SwitchHandle
                  active={formik.values.userFriendly === false}
                ></S.SwitchHandle>
                <S.SwitchText>No</S.SwitchText>
              </S.Switch>
              <S.Switch onClick={() => handleUserFriendly(true)}>
                <S.SwitchHandle
                  active={formik.values.userFriendly}
                ></S.SwitchHandle>
                <S.SwitchText>Yes</S.SwitchText>
              </S.Switch>
            </S.SwitchWrapper>
